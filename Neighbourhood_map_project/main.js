//init will be called when the map loads
function init(){
//error handling functionality studied from http://stackoverflow.com/questions/9228958/how-to-check-if-google-maps-api-is-loaded
    ...
    var viewModel=function(){
      ...
      self.places().forEach(function(place) {
        ...
        marker.addListener('click', function() {
          ...
          //removing animations from all the markers
          mark.setAnimation(null);
          });
          // set image of clicked marker to green
          this.setIcon('http://maps.google.com/mapfiles/ms/icons/green-dot.png');
          //set animation of the clicked marker to bounce
          ...
          //marker animation set to null
          infoWindow.marker.setAnimation(null);
          });
        });
      });

      //calling the filter location function and passing  in the value of check box
      // studied from http://stackoverflow.com/questions/17314610/knockoutjs-when-checked-data-binding-call-function
      this.isChecked.subscribe(function(value){
            this.filterLocations(value);
        }, this);

      this.filterLocations=function(check){
      // filter is a ko.computed because it manipulates values defined before
        if(this.isChecked()===true){
          this.filter=ko.computed(function(){
            ...
            if(!input){
              ...
              self.places().forEach(function(place){
                // if indexOf comparision in if statement is -1 it means nothing matches and check the value of checkbox
                if(place.title().toLowerCase().indexOf(input)==-1 && self.isChecked()){
                   //if text doesn't match it  is not visible
                  place.visibility(false);
                  //if it is not visible marker is set to null
                  place.marker.setMap(null);
                }
                else{
                  // if something(text) matches it  is visible
                  place.visibility(true);
                  // marker is set to map
                  place.marker.setMap(map);
                }
              });
            }
        ...
        //if checkbox value is false show all places and markers
        else{
          self.places().forEach(function(place){
            place.visibility(true);
            place.marker.setMap(map);
...
// this function will be  called when map could not  load studied from http://www.w3schools.com/jsref/event_onerror.asp
function error(){
